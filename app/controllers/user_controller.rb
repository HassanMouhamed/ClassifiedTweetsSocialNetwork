class UserController < ApplicationController
  ...
  def home
    ...
    if params[:category_id]
      if params[:category_id].to_i > 6
        cat_index = "6"
        @posts = Post.find_by_sql("SELECT * FROM posts WHERE post_category > #{cat_index} AND user_id in
        ...
        @posts = @posts.concat(Post.find_by_sql("SELECT * FROM posts WHERE post_category > #{cat_index} AND (is_public = 1 OR user_id = #{current_user_id})  order by image_updated_at DESC LIMIT 10")).uniq

      else

      @posts = Post.find_by_sql("SELECT * FROM posts WHERE post_category = #{params[:category_id]} AND user_id in
			                      (SELECT first_user_id from friendships WHERE second_user_id = #{current_user_id} UNION
			                       SELECT second_user_id from friendships WHERE first_user_id = #{current_user_id}) order by image_updated_at DESC LIMIT 10")
      @posts = @posts.concat(Post.find_by_sql("SELECT * FROM posts WHERE post_category =  #{params[:category_id]} AND (is_public = 1 OR user_id = #{current_user_id})  order by image_updated_at DESC LIMIT 10")).uniq
      end
    else
